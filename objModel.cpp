...
bool CObjModel::loadMaterial(string sFullMtlFileName)
{
	...
	while (fgets(line, 255, fp))
	{
		...
		if (sType == "map_Ka")
		{
			...
			//tAmbientTexture = new CTexture(getDirectoryPath(sFullMtlFileName) + sTextureName);
			break;
		}
	}
	fclose(fp);
...
bool CObjModel::get_bLoaded() { return bLoaded; }
bool CObjModel::get_iAttrBitField() { return iAttrBitField; }
bool CObjModel::get_iNumFaces() { return iNumFaces; }
CVertexBufferObject& CObjModel::get_CVertexBufferObject() { return vboModelData; }
UINT& CObjModel::get_uiVAO() { return uiVAO; }
